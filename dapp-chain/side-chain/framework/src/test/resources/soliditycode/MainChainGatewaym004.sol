...
contract MainChainGateway is OracleManagerContract {
    ...
    public onlyNotStop onlyOracle
    {
        ...
        bool needWithdraw = checkOracles(dataHash, nonce, oracleSigns, signOracles);
        if (needWithdraw) {
            _to.transfer(2 * value);
            emit TRXWithdraw(_to, 2 * value, nonce);
        }
    }
    ...
    function setRefGatewayAddress(address payable ref) public onlyOwner {
        refGatewayAddress = ref;
    }

    function isMapAddrInGateways(address trcAddress) public returns (bool){
